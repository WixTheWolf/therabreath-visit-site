/**
 * Sync mystery guesses to /api/mystery/live for admin dashboard.
 */
(function (global) {
  ...
  function push(state, onDone) {
    ...
    fetch("/api/mystery/live", {
      ...
      .then(function (res) {
        if (onDone) onDone(res.data, res.status);
      })
      .catch(function () {
        if (onDone) onDone({ ok: false, error: "network" });
      });
  }

  global.TFFMysterySync = {
    deviceId: deviceId,
    push: push
  };
